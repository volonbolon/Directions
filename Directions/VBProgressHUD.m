...
#define VBProgressHUDShowNetworkIndicator 1

@interface VBProgressHUD ()

@property (nonatomic, readwrite) VBProgressHUDMaskType maskType;
...
- (void)showWithStatus:(NSString *)string 
              maskType:(VBProgressHUDMaskType)hudMaskType
...
@synthesize hudView; 
@synthesize maskType;
@synthesize showNetworkIndicator;
...
static VBProgressHUD *sharedView = nil;
...
+ (VBProgressHUD *)sharedView {
	
	if(sharedView == nil) {
        sharedView = [[VBProgressHUD alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
    }
...
+ (void)setStatus:(NSString *)string {
	[[VBProgressHUD sharedView] setStatus:string];
}

#pragma mark - Show Methods

+ (void)show {
	[VBProgressHUD showWithStatus:nil 
                 networkIndicator:VBProgressHUDShowNetworkIndicator];
}

+ (void)showWithStatus:(NSString *)status {
    [VBProgressHUD showWithStatus:status
                 networkIndicator:VBProgressHUDShowNetworkIndicator];
}
...
+ (void)showWithStatus:(NSString *)status 
      networkIndicator:(BOOL)show {
    [VBProgressHUD showWithStatus:status 
                         maskType:VBProgressHUDMaskTypeNone
...
+ (void)showWithMaskType:(VBProgressHUDMaskType)maskType 
        networkIndicator:(BOOL)show {
    [VBProgressHUD showWithStatus:nil 
                         maskType:maskType networkIndicator:show];
}
 
+ (void)showWithStatus:(NSString *)status 
              maskType:(VBProgressHUDMaskType)maskType 
      networkIndicator:(BOOL)show {
    [[VBProgressHUD sharedView] showWithStatus:status 
                                      maskType:maskType networkIndicator:show];
}

+ (void)showSuccessWithStatus:(NSString *)string {
    [VBProgressHUD show];
    [VBProgressHUD dismissWithSuccess:string
                           afterDelay:1];
}
...
+ (void)dismiss {
	[[VBProgressHUD sharedView] dismiss];
}

+ (void)dismissWithSuccess:(NSString *)successString {
	[[VBProgressHUD sharedView] dismissWithStatus:successString
                                            error:NO];
}
...
+ (void)dismissWithError:(NSString *)errorString {
	[[VBProgressHUD sharedView] dismissWithStatus:errorString
                                            error:YES];
}
...
- (void)drawRect:(CGRect)rect {
    ...
    switch ( self.maskType ) {
        case VBProgressHUDMaskTypeBlack: {
            [[UIColor colorWithWhite:0 alpha:0.5] set];
            CGContextFillRect(context, self.bounds);
            break;
        }
            
        case VBProgressHUDMaskTypeGradient: {
...
- (void)showWithStatus:(NSString *)string 
    ...
    if(self.maskType != VBProgressHUDMaskTypeNone)
    ...
    if(![self isKeyWindow]) {
        ...
        [[UIApplication sharedApplication].windows enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            ...
            if(window.windowLevel == UIWindowLevelNormal && ![[window class] isEqual:[VBProgressHUD class]]) {
...
- (void)dismissWithStatus:(NSString *)string 
	...
	if(error) {
        self.imageView.image = [UIImage imageNamed:@"VBProgressHUD.bundle/error.png"];
    } else {
        self.imageView.image = [UIImage imageNamed:@"VBProgressHUD.bundle/success.png"];
    }
	...
	fadeOutTimer = [NSTimer scheduledTimerWithTimeInterval:seconds
                                                    target:self 
...
- (CGFloat)visibleKeyboardHeight {
    ...
    @autoreleasepool {
        ...
        for (UIWindow *testWindow in [[UIApplication sharedApplication] windows]) {
            if(![[testWindow class] isEqual:[UIWindow class]] && ![[testWindow class] isEqual:[VBProgressHUD class]]) {
